...
export default function ThemeToggle({ className }: { className?: string }) {
  ...
  if (!mounted) {
    ...
    return <span className={cn('inline-block w-[52px] h-7 rounded-full bg-slate-200/70 dark:bg-slate-700/50 animate-pulse', className)} aria-hidden />;
  }
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isDark
          ? 'bg-slate-700 ring-1 ring-white/8'
          : 'bg-slate-200 ring-1 ring-black/5',
        className,
      )}
    >
      {/* Track icons — always visible, dim when inactive */}
      ...
      <span
        className={cn(
          'absolute top-0.5 left-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full',
          'shadow-md ring-1 transition-all duration-300 ease-in-out will-change-transform',
          isDark
            ? 'translate-x-[26px] bg-slate-900 ring-white/10'
