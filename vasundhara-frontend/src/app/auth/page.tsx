...
export default function AuthLanding() {
  ...
  const roleConfig = {
    household: {
      icon: HomeIcon,
      label: t('role.household', 'Household'),
    ...
    admin: {
      ...
      description: t('role.adminDesc', 'System administration'),
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="hidden lg:block space-y-10 px-4">
                <div className="space-y-6">
                  <div className="flex items-center gap-5">
                    <Link href="/" className="flex items-center gap-5 hover:opacity-85 transition-all duration-200">
                  ...
                  <h2 className="text-5xl font-bold text-slate-800 dark:text-slate-100 leading-[1.15]">
                    {t('auth.tagline1', 'Smart Inventory')}<br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-600 dark:from-emerald-400 dark:to-teal-400">{t('auth.tagline2', 'Management for')}</span><br />
                ...
                <div className="space-y-6">
                  {[
                    { icon: '🏠', text: t('auth.feature1.text', 'Track household essentials effortlessly'), desc: t('auth.feature1.desc', 'Never run out of what you need') },
                    { icon: '📊', text: t('auth.feature2.text', 'Get smart recommendations & analytics'), desc: t('auth.feature2.desc', 'Data-driven decisions for your home') },
                    { icon: '🛒', text: t('auth.feature3.text', 'Connect with local shops & marketplace'), desc: t('auth.feature3.desc', 'Seamless shopping experience') },
                  ].map((item, idx) => (
              ...
              <div className="w-full">
                <div className="bg-white/80 dark:bg-black/80 backdrop-blur-xl rounded-[2.5rem] shadow-2xl shadow-emerald-900/10 dark:shadow-white/5 border border-white/40 dark:border-gray-800/50 p-6 sm:p-10 transition-all duration-300">
                  {/* Mobile logo */}
                  <Link href="/" className="lg:hidden flex items-center gap-4 mb-8 hover:opacity-85 transition-all duration-200">
                    ...
                    <>
                      ...
                      {isLogin ? (
                        <div className="space-y-5">
                          <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 ml-1">{t('auth.emailLabel', 'Email')}</label>
                            <Input
                          ...
                          <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 ml-1">{t('auth.passwordLabel', 'Password')}</label>
                            <div className="relative">
                              <Input
                                placeholder={t('auth.passwordPlaceholderLogin', 'Enter your password')}
                                value={password}
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (!guestName || !guestEmail) {
                                const name = prompt(t('auth.promptGuestName', 'Enter your name for guest mode')) || '';
                                const mail = prompt(t('auth.promptGuestEmail', 'Enter your email for guest mode (optional)')) || '';
                        ...
                        <div className="space-y-5">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 ml-1">{t('auth.firstNameLabel', 'First name')}</label>
                              <Input placeholder={t('auth.firstNamePlaceholder', 'Enter your first name')} value={firstName} onChange={e => setFirstName(e.target.value)} />
                            </div>
                            <div>
                              <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 ml-1">{t('auth.lastNameLabel', 'Last name')}</label>
                              <Input placeholder={t('auth.lastNamePlaceholder', 'Enter your last name')} value={lastName} onChange={e => setLastName(e.target.value)} />
                            </div>
                          </div>

                          <div className="space-y-2">
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 ml-1">{t('auth.profilePictureLabel', 'Profile picture')}</label>
                            <div className="flex flex-wrap items-center gap-4">
                              ...
                              <div className="flex flex-col gap-2">
                                <label className="inline-flex items-center justify-center rounded-lg border border-emerald-200 dark:border-emerald-700 bg-white dark:bg-slate-800 px-4 py-2 text-sm font-semibold text-emerald-700 dark:text-emerald-400 cursor-pointer hover:border-emerald-400 dark:hover:border-emerald-500 transition-colors">
                                  {t('auth.uploadPhoto', 'Upload photo')}
                                ...
                                <p className="text-xs text-slate-500 dark:text-slate-500">{t('auth.photoHint', 'Square images work best. Max 3 MB.')}</p>
                              </div>
                            </div>
                          </div>

                          <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 ml-1">{t('auth.phoneNumberLabel', 'Phone number')}</label>
                            <Input placeholder={t('auth.phoneNumberPlaceholder', '+91 98765 43210')} value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} />
                          </div>

                          <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 ml-1">{t('auth.emailLabel', 'Email')}</label>
                            <Input placeholder={t('auth.emailPlaceholder', 'you@example.com')} value={email} onChange={e => setEmail(e.target.value)} />
                          </div>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <Input
                                placeholder={t('auth.passwordPlaceholderSignup', 'Create a strong password')}
                                value={password}
                                onChange={e => setPassword(e.target.value)}
                          ...
                          {effectiveRole === 'household' && (
                            <div className="space-y-3 bg-gradient-to-br from-emerald-50 to-teal-50 border-2 border-emerald-100 rounded-xl p-4">
                              <p className="text-emerald-900 font-bold text-sm flex items-center gap-2">
                                ...
                                {t('auth.householdDetails', 'Household details')}
                              </p>
                          ...
                          <div className="relative my-6">
                            ...
                            <div className="relative flex justify-center text-sm">
                              <span className="px-4 bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 font-medium">{t('auth.orContinueWith', 'or continue with')}</span>
                            </div>
                          </div>
                          ...
                          <Button
                            ...
                            className="w-full h-12 border-2 border-gray-200 dark:border-gray-800 text-gray-700 dark:text-gray-300 hover:border-emerald-300 dark:hover:border-emerald-600 hover:bg-emerald-50 dark:hover:bg-gray-900 font-semibold rounded-xl transition-all duration-200"
                          >
                            {t('auth.tryGuestButton', 'Try as Guest')}
                          </Button>
                        </div>
                      )}
                    </>
                  ...
                  {user && (
                    <div className="text-center space-y-6">
                      ...
                      <div>
                        <p className="text-xl font-bold text-slate-900 dark:text-white">{t('auth.welcomeBack', 'Welcome back!')}</p>
                ...
                <p className="text-center text-sm text-slate-500 dark:text-slate-500 mt-8">
                  {language === 'hi' ? 'जारी रखकर, आप हमारे ' : language === 'bn' ? 'এগিয়ে যাওয়ার মাধ্যমে, আপনি আমাদের ' : 'By continuing, you agree to our '}
                  <a href="#" className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 font-semibold underline decoration-2 decoration-transparent hover:decoration-emerald-500 transition-all">
                    {t('auth.terms', 'Terms')}
                  </a>
                  {language === 'hi' ? ' और ' : language === 'bn' ? ' এবং ' : ' and '}
                  <a href="#" className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 font-semibold underline decoration-2 decoration-transparent hover:decoration-emerald-500 transition-all">
