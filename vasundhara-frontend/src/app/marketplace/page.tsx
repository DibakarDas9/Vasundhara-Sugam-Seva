...
function MarketplaceContent() {
  ...
  const handleListingSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatusMessage(t('market.form.storedLocally', 'Listing saved locally. Sharing with the community opens soon.'));
    ...
    <div className="flex h-screen bg-gray-50 dark:bg-black">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <Header
          ...
          subtitle={t('market.subtitle', 'Share surplus food and find great deals from your community')}
        />
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6">
            <Card className="border-dashed border-green-500/50 bg-green-50 dark:bg-green-950/20">
              <CardContent className="flex flex-col gap-2 text-center py-6">
                <p className="text-sm font-semibold uppercase tracking-wide text-green-600 dark:text-green-300">
                  {t('market.betaTitle', 'Marketplace beta update')}
                </p>
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">
                  {t('market.betaHeading', 'Collect sample listings while we finish the release')}
                </h3>
                <p className="text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
                  {t('market.betaDescription', "Add your surplus items below to preview how listings will appear. We'll open community-wide exchanges once moderation and pickup flows are finalized.")}
                </p>
              </CardContent>
            </Card>
            ...
            <div ref={listItemSectionRef}>
              <Card>
                <CardHeader>
                  <CardTitle className="text-xl">{t('market.form.title', 'List an item for early access')}</CardTitle>
                </CardHeader>
                <CardContent>
                  <form className="space-y-6" onSubmit={handleListingSubmit}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        {t('market.form.storedLocally', 'Listings are stored locally for now and help us polish the experience.')}
                      </p>
                      <Button type="submit" loading={isSubmitting} disabled={!isFormValid || isSubmitting}>
                        {t('market.form.savePreview', 'Save preview listing')}
                      </Button>
                    </div>
                  </form>
                </CardContent>
            ...
            {filteredListings.length === 0 && (
              <Card className="text-center py-12">
                <CardContent>
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{t('market.emptyTitle', 'No listings yet')}</h3>
                  <p className="text-gray-600 dark:text-gray-400 mb-4">
                    {t('market.emptyDetail', 'Use the form above to add your first preview listing while we finish the public rollout.')}
                  </p>
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    {t('market.emptyHint', 'Everything stays on your device until the marketplace opens.')}
                  </div>
                </CardContent>
              </Card>
            )}
